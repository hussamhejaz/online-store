    ...
    <main dir="rtl" className="min-h-screen p-6 md:p-10">
      <div className="mx-auto w-full max-w-5xl space-y-6">
        <header className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <h1 className="mt-1 text-2xl font-semibold">إضافة منتج جديد</h1>
        </header>

        <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          {isLoading ? (
            <p className="text-sm text-zinc-500">جاري تحميل التصنيفات...</p>
          ) : (
            <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
              ...
              <input className="rounded-md border border-zinc-300 px-3 py-2.5 text-sm outline-none ring-rose-500 focus:ring-2" placeholder="slug (اختياري)" value={form.slug} onChange={(e) => setForm((s) => ({ ...s, slug: e.target.value }))} />
              ...
              <input className="rounded-md border border-zinc-300 px-3 py-2.5 text-sm outline-none ring-rose-500 focus:ring-2" placeholder="سعر المقارنة (اختياري)" type="number" min="0" step="0.01" value={form.compareAtPrice} onChange={(e) => setForm((s) => ({ ...s, compareAtPrice: e.target.value }))} />
              ...
              <div className="grid gap-2 md:col-span-2">
                <input className="rounded-md border border-zinc-300 px-3 py-2.5 text-sm outline-none ring-rose-500 focus:ring-2" placeholder="رابط الصورة (اختياري)" value={form.image} onChange={(e) => setForm((s) => ({ ...s, image: e.target.value }))} />
                <input
                  type="file"
              ...
              <textarea className="min-h-24 rounded-md border border-zinc-300 px-3 py-2.5 text-sm outline-none ring-rose-500 focus:ring-2 md:col-span-2" placeholder="وصف المنتج (اختياري)" value={form.description} onChange={(e) => setForm((s) => ({ ...s, description: e.target.value }))} />
              ...
              <button disabled={isSaving} type="submit" className="w-fit rounded-md bg-rose-600 px-4 py-2 text-sm font-medium text-white hover:bg-rose-500 disabled:opacity-60">
